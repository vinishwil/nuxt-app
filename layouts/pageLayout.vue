  ...
  <v-app id="inspire">
    <v-navigation-drawer v-model="drawer" app clipped color="grey lighten-4">
      ...
      >
        <v-row align="end" class="lightbox black--text pa-2 fill-height">
          <v-col>
            <div class="subheading"><strong>Vinish Wilson</strong></div>
            <div class="body-1"><strong>vinish.wm@gmail.com</strong></div>
          </v-col>
        </v-row>
      </v-img>
      <v-list dense>
        <template v-for="item in items">
          ...
          <v-list-item v-else :key="item.text" link @click="openPage">
            <v-list-item-action>
    ...
    >
      ...
      <v-btn icon large @click="logOut">
        <v-avatar size="32px" item> <v-img :src="image" alt="User"/></v-avatar>
      </v-btn>
    </v-app-bar>
    <v-content>
...
export default {
  ...
  data: () => ({
    ...
    drawer: null,
    items: [
      { icon: 'mdi-contacts', text: 'Contacts', link: 'home' },
  ...
  methods: {
    openPage() {
      const route = this.$router.resolve({ path: '/home' })
